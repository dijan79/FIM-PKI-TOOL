...
The GUI uses Tkinter, which needs access to a display server. The
instructions below cover Linux (most common for coursework/demo setups).

### Linux (X11)

1. **Allow Docker to access your display** (once per session):
   ```bash
   xhost +local:docker
   ```
...
2. **Build and run** with Docker Compose:
   ...
   ```
   This builds the image, mounts `keys/ certs/ data/ logs/ CSV_logs/`
   to your host so PKI material and logs persist, and forwards your X11
   display so the window appears on your desktop.
...
### Monitoring a folder on your host machine

Mount it as a volume. Edit `docker-compose.yml` and uncomment/edit:
```yaml
    - /path/on/host/to/watch:/watched
```
Then, inside the app's **Live Monitor** page, browse to `/watched` —
changes to that host folder will be detected by the container.

### Manual `docker run` (alternative to compose)

```bash
docker build -t fim-pki-sentinel .
...
docker run -it --rm \
  ...
  -v $(pwd)/logs:/app/logs \
  -v $(pwd)/CSV_logs:/app/CSV_logs \
  fim-pki-sentinel
```

### Windows / macOS

Running Linux GUI apps from Docker on Windows/macOS requires an X server
on the host (e.g. **VcXsrv** on Windows, **XQuartz** on macOS) with
`DISPLAY` pointed at it. If your coursework only requires demonstrating
that the project **builds and runs in Docker**, you can show:
```bash
docker build -t fim-pki-sentinel .
docker run --rm fim-pki-sentinel python -c "import fim_gui; print('Build OK')"
```
as evidence the containerised environment is correctly configured, and
run the GUI natively (`python fim_gui.py`) for the live demo.
